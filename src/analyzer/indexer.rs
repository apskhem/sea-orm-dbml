...
impl Indexer {
  pub fn index_table(&mut self, tables: &Vec<table::TableBlock>) {
    for table in tables.iter() {
      ...
      if let Some(indexes_block) = table.indexes.clone() {
        panic!("indexes_block is unsupported")
      }
  ...
  pub fn index_enums(&mut self, enums: &Vec<enums::EnumBlock>) {
    ...
    if let Some(block) = self.schema_map.get(&schema) {
      if let Some(col_set) = block.table_map.get(table) {
        for v in fields.iter() {
          if !col_set.contains(v) {
            return Err(format!("not found '{}' column in table '{}'", v, table));
          }
        }
