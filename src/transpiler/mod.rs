...
fn transpile_sea_orm_postgresql(ast: analyzer::SemanticSchemaBlock) -> Result<String> {
  ...
  let codegen = ast.tables.iter().fold(codegen, |acc, table| {
    ...
    let table_block = fields.into_iter().fold(table_block,|acc, field| {
      ...
      if let Some(exp_type) = field.r#type.type_name.to_col_type(&field.r#type.args) {
      ...
      let field_rust_type = field.r#type.type_name.to_rust_sea_orm_type();
